...
function mapCircle(row: Record<string, unknown>): ConversationCircle {
  ...
  return {
    ...
    title: getCell(row, "AUDIENCE_LABEL") || "Family Circle",
...
export async function getConversationCircleForPerson(circleId: string, personId: string): Promise<ConversationCircle | null> {
  ...
  return withConnection(async (rawConnection) => {
    ...
    const result = await connection.execute(
      ...
      { circleId: normalizedCircleId, personId: viewerPersonId },
      OUT_FORMAT,
    );
...
async function findCircleBySignature(connection: DbConnection, signature: string, memberCount: number) {
  const result = await connection.execute(
    `SELECT *
     FROM (
       SELECT
       ...
       WHERE LOWER(TRIM(NVL(t.thread_status, 'active'))) <> 'archived'
         AND LOWER(TRIM(NVL(m.is_active, 'TRUE'))) <> 'false'
       GROUP BY t.thread_id, t.family_group_key, t.audience_label, t.created_at, t.updated_at, t.last_post_at
...
export async function createConversationCircle(input: {
  ...
  const title = normalize(input.title) || "Family Circle";
  const members = uniquePersonIds([actorPersonId, ...input.memberPersonIds]);
  if (!actorPersonId || members.length < 2) {
    throw new Error("circle_requires_at_least_two_people");
  }
  ...
  return withConnection(async (rawConnection) => {
    ...
    const existing = await findCircleBySignature(connection, signature, members.length);
    if (existing) {
      const membersByCircle = await listMembersForCircles(connection, [existing.circleId]);
      return { ...existing, members: membersByCircle.get(existing.circleId) ?? [] };
    }
    ...
    try {
      await connection.execute(
         ...
         )`,
        {
          ...
          audienceType: "person_circle",
          audienceKey: signature,
          title,
    ...
    if (!created) throw new Error("created_circle_not_found");
    return created;
  });
}
...
export async function createCircleConversation(input: {
  ...
  if (!circle) throw new Error("circle_not_found_or_not_member");
...
export async function listPersonConversationSummaries(input: {
  ...
  return withConnection(async (rawConnection) => {
    const connection = rawConnection as DbConnection;
    await ensureShareTables(connection);
    const binds: Record<string, unknown> = { viewerPersonId };
    ...
    for (const row of result.rows ?? []) {
      ...
      current.push({
        ...
        conversationId: getCell(row, "CONVERSATION_ID"),
        circleId: getCell(row, "THREAD_ID"),
        circleTitle: getCell(row, "CIRCLE_TITLE") || "Family Circle",
        title: getCell(row, "TITLE") || "Conversation",
