...
import { z } from "zod";
import { listOwnerShareDefaults, replaceOwnerShareDefaults } from "@/lib/u1/access-store";
...
export async function GET() {
  ...
  const rows = await listOwnerShareDefaults(resolved.actorPersonId);
...
export async function PUT(request: Request) {
  ...
  await replaceOwnerShareDefaults(resolved.actorPersonId, parsed.data.rules);
  triggerViewerRecompute(resolved.actorPersonId, "owner_share_defaults_updated");
  const rows = await listOwnerShareDefaults(resolved.actorPersonId);
  return NextResponse.json({
    ownerPersonId: resolved.actorPersonId,
