...
- [ ] Famailink person-based Conversations MVP from EFL Shares
  ...
  - Completed local implementation 2026-05-18: refined Share so Add Group individual-member results stay hidden until the user types a search string, separated thread-level `Files` and `Camera` entry points, added inline video preview/playback in Share threads, and synced the required OCI object-storage env/auth vars into `famailink-mvp` for deployed media-upload readiness.
  - Implementation plan 2026-06-14: add Share group and thread management controls.
    - Scope: allow a signed-in group member to rename the selected Group, allow a signed-in conversation member to rename a thread, and allow a signed-in conversation member to archive/delete a thread from the active Group.
    - API/UI/data changes: reuse existing `share_threads.audience_label` / `share_thread_members.group_display_name` for Group rename, add conversation PATCH/DELETE handlers that update `share_conversations.title` or archive the conversation, and add compact edit/delete actions with confirmation in the existing single-level Share UI.
...
- [ ] Legacy media/share compatibility hard cutover + test-content reset
  ...
  - Disabled destructive reset apply mode; dry-run reporting remains available while a targeted reset plan is defined.
  Est date: 2026-04-05
...
- [x] Unit 1 greenfield lab app / Famailink MVP (isolated, no legacy feature imports)
  ...
  Cleanup rationale:
  - Reproduction path: Open TODO.md and the top Active item still shows the Unit 1/Famailink MVP as in progress despite multiple deployed completion entries.
  - Failing tracking path: Completed implementation plans remained nested under the active parent item, so the project tracker did not clearly expose the next real priority.
  - Root cause: Unit 1/Famailink was delivered incrementally, and post-deploy TODO hygiene marked individual slices but never closed the parent MVP item.
  - Why this cleanup fixes it: The active list now removes the completed parent and keeps one concise completed record with follow-up notes instead of carrying the full deployed execution history at the top of TODO.md.
  Summary:
  - Built the isolated Famailink app with local login/session, password reset, preferences, preview, recompute/status, relationship rules, tree view, and person-specific settings.
  - Added broad inclusive defaults, explicit supported in-law categories, Administration/Rules Tree surfaces, EFL-style household/focused tree behavior, and person detail tabs for Overview, Vitals, Media, Stories, Conversations, and Updates & Sharing.
  - Added a repeatable Famailink production validation command and runbook, plus root build-scope cleanup so nested standalone apps are validated through their own package builds.
  Verification:
  - Famailink implementation slices were linted, built, committed, and deployed through the entries referenced in docs/change-summary.md and changeHistory.md.
  - Root production build and deployed smoke checks passed after the 2026-04-21 build-scope and warning-cleanup deploys.
  Follow-up notes:
  - Stories and Conversations are intentional MVP tab shells; populate them as separate future product tasks when ready.
  - Local direct efl2 build still hits a Windows spawn EPERM before compilation; this is a local worker-spawn follow-up, not a blocker for the completed Famailink MVP or root production build.
