...
- 2026-03-22: Face recognition now uses one global person/file identity model: canonical `person_face_profiles`, `face_instances`, and `face_matches` are stored under a global scope sentinel, and family access is enforced when suggestions are read instead of by duplicating biometric rows per family.
- 2026-03-22: Manual reviewed face-to-person association is now the primary way to build trustworthy person face profiles: the app should show the detected face crop, require an explicit person choice, and only then write the face embedding onto that person's profile.
- 2026-03-22: High-value normalized EXIF fields are now persisted directly on `MediaAssets`, with `exif_extracted_at` marking that EXIF collection has already been attempted so later intelligence runs can reuse stored EXIF instead of reparsing the image.
- 2026-03-23: OCI Vision `analyzeImage` now uses a direct signed REST request in runtime code while keeping the OCI SDK for auth/signing/http transport, because the generated `AIServiceVisionClient.analyzeImage(...)` wrapper was unreliable on some valid images.
- 2026-03-24: Asset-level technical media fields now belong on normalized `MediaAssets` columns (`source_provider`, `source_file_id`, object keys, checksum, dimensions, duration), and `MediaLinks` should not store copied asset technical metadata.
- 2026-03-24: The media modal is now a stored-detail and stored-snapshot surface only; it must not auto-run or expose active intelligence, EXIF refresh, processing-status refresh, or face-association actions until the intelligence workflow is redesigned.
