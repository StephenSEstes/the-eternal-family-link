...
- `Area`: Famailink person detail content tabs
- `Decision`: The `/tree` person detail modal should expose the fuller tab structure now: Overview, Vitals, Media, Stories, Conversations, and Updates & Sharing. Vitals and Media are viewer-facing content tabs gated by what the selected person makes available to the signed-in viewer; Updates & Sharing remains the outbound settings tab for what the signed-in viewer shares with the selected person. Stories and Conversations are tab shells only in this MVP pass.
- `Reason`: The privacy/sharing model is useful only if the person modal begins showing real content one scope at a time. Vitals and Media are the next practical scopes, while building stories or conversations now would expand the MVP beyond the immediate application-running goal.
